...
import { MonitorPlay, Music, Tv2, Flame, Mic2, Search } from 'lucide-react';
...
export default function HomePage() {
  ...
  return (
    <div className="space-y-12 pb-12">
      {/* Cinematic Hero */}
      <section className="relative overflow-hidden rounded-[2.5rem] border border-white/5 bg-[#0f172a]/40 bg-[url('https://images.unsplash.com/photo-1514525253161-7a46d19cd819?q=80&w=2874&auto=format&fit=crop')] bg-cover bg-center px-6 py-20 text-center shadow-2xl backdrop-blur-3xl md:px-12 md:py-32">
        <div className="absolute inset-0 bg-gradient-to-b from-[#020617]/80 via-[#020617]/90 to-[#020617]" />
        
        {/* Floating Icons Background */}
        <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-20">
            <motion.div animate={{ y: [0, -20, 0] }} transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut' }} className="absolute top-10 left-10 text-cyan-500"><MonitorPlay size={48} /></motion.div>
            <motion.div animate={{ y: [0, 20, 0] }} transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut', delay: 1 }} className="absolute bottom-20 left-1/4 text-fuchsia-500"><Music size={64} /></motion.div>
            <motion.div animate={{ y: [0, -15, 0] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut', delay: 2 }} className="absolute top-20 right-1/4 text-violet-500"><Tv2 size={40} /></motion.div>
            <motion.div animate={{ y: [0, 15, 0] }} transition={{ duration: 7, repeat: Infinity, ease: 'easeInOut', delay: 1.5 }} className="absolute bottom-10 right-10 text-amber-500"><Mic2 size={56} /></motion.div>
        </div>
        ...
        <div className="relative z-10 mx-auto max-w-3xl space-y-6">
          ...
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="text-4xl font-extrabold tracking-tight text-white md:text-6xl lg:text-7xl">
            Your Front Row Seat to <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-violet-500 to-fuchsia-500">Everything</span>
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="mx-auto max-w-2xl text-lg text-slate-400 md:text-xl">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between py-2">
          ...
          <div className="flex flex-wrap gap-2 overflow-x-auto pb-2 -mb-2 md:pb-0 md:mb-0 hide-scrollbar scroll-smooth snap-x">
            {categories.map((cat) => {
              ...
              // Determine active theme classes dynamically based on category
      ...
      {/* Events Grid */}
      <motion.div 
        key={activeCategory + search} // Force re-mount animation on filter change
